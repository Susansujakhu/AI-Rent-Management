...
import { ElectricityPortalClient, type PortalReading } from "../../electricity/electricity-client";
...
export default async function PortalElectricityPage({
  ...
  if (!tenant.canSubmitMeterReading) {
    return (
      <PortalShell tenantName={tenant.name} roomName={tenant.room?.name ?? null} showElectricity={false} token={token}>
